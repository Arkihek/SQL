...
WHERE veli_isim = 'Ayse' OR veli_isim = 'Ali';
-- 2. YONTEM
DELETE FROM cucuklar
WHERE veli_isim IN ('Ayse','Ali');
...
DELETE FROM cocuk; -- olusturdugumuz yeni bir alana database`imizin bir kopyasini koyduk

ROLLBACK TO cocuk1; -- bu satirda ise yaptigimiz degisiklikleri savepoint cocuk1 durumundaki haline geri donduruyoruz
ROLLBACK TO cocuk2; 
...
CREATE TABLE talebeler
    ...
    REFERENCES talebeler(id) ON DELETE CASCADE -- parenttan silme islemi yapinca child`da siler bu komutu yazdigimizda
    ); 
